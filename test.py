...
from plot import backtest_stats, convert_daily_return_to_pyfolio_ts
...
# #Load default 
# df_daily_return_a2c = pd.read_csv("daily_return_a2c.csv")
# df_daily_return_ppo = pd.read_csv("daily_return_ppo.csv")
# df_actions_ppo = pd.read_csv("df_action_ppo.csv")
# df_actions_a2c = pd.read_csv("df_action_a2c.csv")
...
trained_a2c = agent.load_model("a2c", "trained_models/trained_a2c_vn.zip")
...
for algo in {"A2C", "PPO"}:
    ...
    for i in range(0, len(unique_trade_date) - 1):
...
for algo in ["LR", "RF", "Reference Model", "SVM", "DT"]:
    if algo == "LR":
        weights = lr_weights
    elif algo == "RF":
        weights = rf_weights
...
for i in range(0, len(unique_trade_date) - window):
    ...
    for algo in ["LR", "RF", "DT", "SVM", "PPO", "A2C"]:
...
for name in ["LR", "RF", "Reference Model", "SVM", "DT", "A2C", "PPO"]:
    if name == "DT":
...
for algo in ["LR", "DT", "RF", "SVM", "A2C", "PPO"]:
